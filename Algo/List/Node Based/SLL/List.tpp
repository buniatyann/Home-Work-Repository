#ifndef SINGLE_LIST_TPP
#define SINGLE_LIST_TPP

#include "List.hpp"

template <typename T>
Node<T>::Node(const T& val, Node* next) : val(val), next(next) {}

template <typename T>
Node<T>::Node(T&& val, Node* next) : val(std::move(val)), next(next) {}
...
void List<T>::insert(T&& value, std::size_t pos) {
    ...
    Node<T>* new_node = new Node<T>(std::move(value), current->next);
    current->next = new_node;
    if (new_node->next == nullptr) {
        back_ = new_node;
    }
...
void List<T>::push_front(const T& value) {
    Node<T>* new_node = new Node<T>(value, front_);
    front_ = new_node;
    if (!back_) {
        back_ = new_node;
    }
...
void List<T>::push_front(T&& value) {
    Node<T>* new_node = new Node<T>(std::move(value), front_);
    front_ = new_node;
    if (!back_) {
        back_ = new_node;
    }
