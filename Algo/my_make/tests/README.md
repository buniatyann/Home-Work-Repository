...
Folders are grouped by responsibility rather than by individual class:

```
src/
├── core/         Shared types: SourceLocation, Config, error hierarchy
├── parser/       Front end — lexer, parser, AST data structures
├── eval/         Middle end — variable table (+ future evaluator, functions)
├── engine/       Back end — rule database, dependency graph, executor
└── cli/          Entry point: argument parsing + main.cpp
```
