...
public:
    ...
    explicit memory_pool(size_type initial_size, size_type block_size_in_bytes = 1024, const Alloc& alloc = Alloc());
    ~memory_pool();

    void expand_pool(size_type additional_blocks, size_type block_size = 0);
...
private:
    ...
    using free_blocks_container = std::vector<block_node_t>;
    using using_blocks_container = std::list<block_node_t>;
    ...
    Alloc alloc_;
    mutable std::mutex mutex; // Mutex to protect access to shared resources
