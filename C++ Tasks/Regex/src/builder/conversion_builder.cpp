#include "../../include/builder/conversion_builder.hpp"
    ...
    for (const auto& [quantifier_str, pattern] : regex->subpatterns) {
        ...
        if (quant == quantifier::asterisk || quant == quantifier::lazy_asterisk) {
            ...
            if (quant == quantifier::lazy_optional) {
                base->unshift_transition(base->initial_state(), base->ending_state()[0], epsilon_matcher);
            } 
            else {
                base->add_transition(base->initial_state(), base->ending_state()[0], epsilon_matcher);
            }
        } 
        else {
            base = base_builder();
        ...
        if (!nfa) {
            nfa = std::move(base);  // ✅ Correct move
        } 
        else {
            nfa->thompson_append_nfa(std::move(base), nfa->ending_state()[0]);  // ✅ Correct move
        }
    }

    if (group_name != -1) {
        nfa->add_group(nfa->initial_state(), nfa->ending_state()[0], group_name);
    }
...
//         } 
//         else {
//             nfa->thompson_append_nfa(base, nfa->ending_state()[0]);
//         }
//     }
...
std::unique_ptr<engine_nfa> conversion_builder::asterisk_plus(std::function<std::unique_ptr<engine_nfa>()> builder, bool lazy, bool asterisk) {
    ...
    if (lazy) {
        ...
        base->add_transition(new_init, base->initial_state(), epsilon_matcher);
        base->add_transition(base->ending_state()[0], new_end, epsilon_matcher);
        base->add_transition(base->ending_state()[0], base->initial_state(), epsilon_matcher);
    } 
    else {
        base->add_transition(new_init, base->initial_state(), epsilon_matcher);
        base->add_transition(base->ending_state()[0], base->initial_state(), epsilon_matcher);
...
//         ending_states.insert(ending_states.end(), tmp->ending_state().begin(), tmp->ending_state().end());
