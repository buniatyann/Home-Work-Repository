    ...
    for (std::size_t i = 0; i < triangles.size(); ++i) {
        ...
        for (int e = 0; e < 3; ++e) {
            ...
            if (!found_neighbor) {
                ...
                T dx = static_cast<T>(p2[0]) - static_cast<T>(p1[0]);
                T dy = static_cast<T>(p2[1]) - static_cast<T>(p1[1]);

                // Perpendicular direction
                point_t perp{-dy, dx};

                // Determine outward direction (away from third vertex)
                point_t third = tri.vertices[(e + 2) % 3];
                point_t mid{(static_cast<T>(p1[0]) + static_cast<T>(p2[0])) / 2,
                            (static_cast<T>(p1[1]) + static_cast<T>(p2[1])) / 2};

                T to_third_x = static_cast<T>(third[0]) - static_cast<T>(mid[0]);
                T to_third_y = static_cast<T>(third[1]) - static_cast<T>(mid[1]);

                T dot = static_cast<T>(perp[0]) * to_third_x + static_cast<T>(perp[1]) * to_third_y;
                if (dot > 0) {
...
std::size_t Voronoi<T>::locate(const point_t& p) const {
    ...
    for (std::size_t i = 0; i < cells_.size(); ++i) {
        T dx = static_cast<T>(p[0]) - static_cast<T>(cells_[i].site[0]);
        T dy = static_cast<T>(p[1]) - static_cast<T>(cells_[i].site[1]);
...
std::vector<typename Voronoi<T>::point_t> Voronoi<T>::nearest_neighbors(
    ...
    for (const auto& cell : cells_) {
        T dx = static_cast<T>(p[0]) - static_cast<T>(cell.site[0]);
        T dy = static_cast<T>(p[1]) - static_cast<T>(cell.site[1]);
        T dist_sq = dx * dx + dy * dy;
...
void Voronoi<T>::clip_to_bounds(T margin) {
    ...
    // Add margin
    point_t min_pt = bounds.min();
    point_t max_pt = bounds.max();

    T size_x = static_cast<T>(max_pt[0]) - static_cast<T>(min_pt[0]);
    T size_y = static_cast<T>(max_pt[1]) - static_cast<T>(min_pt[1]);
    T expand = std::max(size_x, size_y) * margin + static_cast<T>(1);
...
typename Voronoi<T>::point_t Voronoi<T>::clip_infinite_edge(
    ...
    point_t origin = e.p1_;
    point_t dir = e.direction_;

    // Normalize direction
    double len = std::sqrt(static_cast<double>(dir[0]) * static_cast<double>(dir[0]) +
                           static_cast<double>(dir[1]) * static_cast<double>(dir[1]));
    if (len < TOLERANCE) {
        return origin;
    }

    T dx = static_cast<T>(static_cast<double>(dir[0]) / len);
    T dy = static_cast<T>(static_cast<double>(dir[1]) / len);
    ...
    if (std::abs(static_cast<double>(dx)) > TOLERANCE) {
        T t1 = (static_cast<T>(min_pt[0]) - static_cast<T>(origin[0])) / dx;
        T t2 = (static_cast<T>(max_pt[0]) - static_cast<T>(origin[0])) / dx;
        if (t1 > 0) t_max = std::min(t_max, t1);
        if (t2 > 0) t_max = std::min(t_max, t2);
    }

    if (std::abs(static_cast<double>(dy)) > TOLERANCE) {
        T t1 = (static_cast<T>(min_pt[1]) - static_cast<T>(origin[1])) / dy;
        T t2 = (static_cast<T>(max_pt[1]) - static_cast<T>(origin[1])) / dy;
        if (t1 > 0) t_max = std::min(t_max, t1);
        if (t2 > 0) t_max = std::min(t_max, t2);
    }

    // Limit t_max to a reasonable value
    T bound_size = std::max(static_cast<T>(max_pt[0]) - static_cast<T>(min_pt[0]),
...
void Voronoi<T>::clip_cell(Cell& cell, const BoundingBox<T, 2>& bounds) {
    ...
    auto clip_polygon_to_edge = [](std::vector<point_t>& polygon,
        ...
        T ex = static_cast<T>(edge_end[0]) - static_cast<T>(edge_start[0]);
        T ey = static_cast<T>(edge_end[1]) - static_cast<T>(edge_start[1]);

        auto inside = [&](const point_t& p) {
            T px = static_cast<T>(p[0]) - static_cast<T>(edge_start[0]);
            T py = static_cast<T>(p[1]) - static_cast<T>(edge_start[1]);
            return ex * py - ey * px >= 0;
        };

        auto intersect_line = [&](const point_t& p1, const point_t& p2) {
            T x1 = static_cast<T>(p1[0]), y1 = static_cast<T>(p1[1]);
            T x2 = static_cast<T>(p2[0]), y2 = static_cast<T>(p2[1]);
            T x3 = static_cast<T>(edge_start[0]), y3 = static_cast<T>(edge_start[1]);
            T x4 = static_cast<T>(edge_end[0]), y4 = static_cast<T>(edge_end[1]);
